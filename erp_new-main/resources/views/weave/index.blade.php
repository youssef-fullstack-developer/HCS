...
@section('content')
    <div class="card card-custom" style="box-shadow: none">
        <div class="card-header flex-wrap border-0 pt-0 pb-0">
            ...
            <div class="card-toolbar">
                <button data-toggle="modal" data-target="#addWeaveModel"
        ...
        <div class="card-body">
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                ...
                <tbody>
                    @if (!empty($weave))
                        @foreach ($weave as $key => $w)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $w->name }}</td>
                                <td>{{ $w->status }}</td>
                                <td class="d-flex">
                                    <a href="{{ route('weave.edit', $w->id) }}"
                                        class="btn btn-warning btn-sm btn-clean btn-icon" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                    {{ html()->form('DELETE', '/weave/' . $w->id)->open() }}
    ...
    <div class="modal fade" id="addWeaveModel" tabindex="-1" role="dialog"
        aria-labelledby="addCertificationModelLabel" aria-hidden="true">
        {{ html()->form('POST', '/weave')->open() }}
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Weave</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group row">
                        <div class="col-12">
                            <label>No of Heald Frame:</label>
                            <input type="text" name="no_of_heald_frame" class="form-control" id="name" autocomplete="off"
                                   placeholder="Enter Name" value="{{ old('no_of_heald_frame') }}"  />
                        </div>
                    </div>
