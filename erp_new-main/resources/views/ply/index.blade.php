...
@section('content')
    <div class="card card-custom" style="box-shadow: none">
        ...
        <div class="card-body">
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                ...
                <tbody>
                    @if (!empty($ply))
                        @foreach ($ply as $key => $p)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $p->ply }}</td>
                                <td>{{ $p->status }}</td>
                                <td class="d-flex">
                                    <a href="{{ route('certification.edit', $p->id) }}"
                                        class="btn btn-warning btn-sm btn-clean btn-icon" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                    {{ html()->form('DELETE', '/certification/' . $p->id)->open() }}
    ...
    <div class="modal fade" id="addCertificationModel" tabindex="-1" role="dialog"
        ...
        {{ html()->form('POST', '/ply')->open() }}
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Ply</h5>
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <div class="col-12">
                            <label>Ply:</label>
                            <input type="text" name="ply" class="form-control" id="ply" autocomplete="off"
                                placeholder="Enter Ply" value="{{ old('ply') }}" required />
                        </div>
                    </div>
