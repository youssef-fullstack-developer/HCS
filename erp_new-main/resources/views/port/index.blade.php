    ...
    <div class="card card-custom" style="box-shadow: none">
        <div class="card-header flex-wrap border-0 pt-0 pb-0">
            ...
            <div class="card-toolbar">
                <a href="{{ route('port.create') }}" class="btn btn-primary font-weight-bolder mr-2">
                    ...
                    @if (!empty($ports))
                        @foreach ($ports as $key => $port)
                            <tr>
                                ...
                                <td>{{ $port->status }}</td>
                                <td class="d-flex">
                                    <a href="{{ route('certification.edit', $port->id) }}"
                                        class="btn btn-warning btn-sm btn-clean btn-icon" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                    {{ html()->form('DELETE', '/certification/' . $port->id)->open() }}
