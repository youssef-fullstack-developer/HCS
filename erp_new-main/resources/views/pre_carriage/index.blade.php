...
@section('content')
                    ...
                    @if (!empty($pres))
                        @foreach ($pres as $key => $pre)
                            <tr>
                                ...
                                <td>{{ $pre->status }}</td>
                                <td class="d-flex">
                                    <a href="{{ route('pre_carriage.edit', $pre->id) }}"
                                        class="btn btn-warning btn-sm btn-clean btn-icon" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                    {{ html()->form('DELETE', '/pre_carriage/' . $pre->id)->open() }}
    ...
    <div class="modal fade" id="addCertificationModel" tabindex="-1" role="dialog"
        ...
        {{ html()->form('POST', '/pre_carriage')->open() }}
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Pre Carriage</h5>
