    ...
    <div class="card card-custom" style="box-shadow: none">
        <div class="card-header flex-wrap border-0 pt-0 pb-0">
            ...
            <div class="card-toolbar">
                <a href="{{ route('selvedge.create') }}" class="btn btn-primary font-weight-bolder mr-2">
            ...
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                <thead>
                    <tr class="text-uppercase">
                        <th>id</th>
                        <th class="pl-7"><span class="text-dark-75">Selvedge Name</span></th>
                ...
                <tbody>
                    @if (!empty($selvedges))
                        @foreach ($selvedges as $key => $selvedge)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $selvedge->name }}</td>
                                <td>{{ $selvedge->status }}</td>
                                <td class="d-flex">
                                    <a href="{{ route('selvedge.edit', $selvedge->id) }}"
                                        class="btn btn-warning btn-sm btn-clean btn-icon" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                    {{ html()->form('DELETE', '/selvedge/' . $selvedge->id)->open() }}
