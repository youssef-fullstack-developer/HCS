            ...
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                <thead>
                    <tr class="text-uppercase">
                        <th>id</th>
                        <th class="pl-7"><span class="text-dark-75">GST Type</span></th>
                        <th class="pl-7"><span class="text-dark-75">IGST%</span></th>
                        <th class="pl-7"><span class="text-dark-75">SGST</span></th>
                        <th class="pl-7"><span class="text-dark-75">CGST</span></th>
                ...
                <tbody>
                    @if (!empty($gsts))
                        @foreach ($gsts as $key => $gst)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $gst->gst_type == 1 ? 'IGST' : 'CGST/SGST'}}</td>
                                <td>{{ $gst->igst }}</td>
                                <td>{{ $gst->sgst }}</td>
                                <td>{{ $gst->cgst }}</td>
                                <td>{{ $gst->status }}</td>
                                <td class="d-flex">
                                    <a href="{{ route('gst.edit', $gst->id) }}"
                                        class="btn btn-warning btn-sm btn-clean btn-icon" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                    {{ html()->form('DELETE', '/gst/' . $gst->id)->open() }}
