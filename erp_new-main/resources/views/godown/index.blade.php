    ...
    <div class="card card-custom" style="box-shadow: none">
        <div class="card-header flex-wrap border-0 pt-0 pb-0">
            <div class="card-title">
                <h3 class="card-label">
                    Go Down Locations List
                </h3>
            </div>
            <div class="card-toolbar">
                <a href="{{ route('godown.create') }}" class="btn btn-primary font-weight-bolder mr-2">
            ...
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                <thead>
                    <tr class="text-uppercase">
                        <th>id</th>
                        <th class="pl-7"><span class="text-dark-75">GODOWN LOCATION</span></th>
                        <th class="pl-7"><span class="text-dark-75">GODOWN DESCRIPTION</span></th>
                    ...
                    @if (!empty($godowns))
                        @foreach ($godowns as $key => $godown)
                            <tr>
                                ...
                                <td>{{ $godown->status }}</td>
                                <td class="d-flex">
                                    <a href="{{ route('godown.edit', $godown->id) }}"
                                        class="btn btn-warning btn-sm btn-clean btn-icon" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                    {{ html()->form('DELETE', '/godown/' . $godown->id)->open() }}
