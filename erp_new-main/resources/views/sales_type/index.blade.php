...
@section('content')
    <div class="card card-custom" style="box-shadow: none">
        ...
        <div class="card-body">
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                ...
                <tbody>
                    @if (!empty($sales_types))
                        @foreach ($sales_types as $key => $sales_type)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $sales_type->name }}</td>
                                <td>{{ $sales_type->status }}</td>
                                <td class="d-flex">
                                    <a href="{{ route('sales_type.edit', $sales_type->id) }}"
                                        class="btn btn-warning btn-sm btn-clean btn-icon" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                    {{ html()->form('DELETE', '/sales_type/' . $sales_type->id)->open() }}
    ...
    <div class="modal fade" id="addCertificationModel" tabindex="-1" role="dialog"
        ...
        {{ html()->form('POST', '/sales_type')->open() }}
                    ...
                    <div class="form-group row">
                        <div class="col-12">
                            ...
                            <select name="account_group" class="form-control" required>
                                <option>Select A/C Group</option>
