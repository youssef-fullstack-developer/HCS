...
@section('content')
                    ...
                    @if (!empty($types))
                        @foreach ($types as $key => $type)
                            <tr>
                                ...
                                <td>{{ $type->status }}</td>
                                <td class="d-flex">
                                    <a href="{{ route('certification.edit', $type->id) }}"
                                        class="btn btn-warning btn-sm btn-clean btn-icon" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                    {{ html()->form('DELETE', '/certification/' . $type->id)->open() }}
    ...
    <div class="modal fade" id="addCertificationModel" tabindex="-1" role="dialog"
        ...
        {{ html()->form('POST', '/loom_types')->open() }}
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Loom Type</h5>
